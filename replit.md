...
### UI/UX Decisions
The application features a bright, cheerful rainbow background design with playful aesthetics. The design uses a vibrant rainbow landscape image (sky blue background with grass, flowers, and sun) as the main backdrop with a semi-transparent dark overlay (40% in light mode, 60% in dark mode) to ensure readability. The color palette includes vibrant rainbow colors (red, orange, yellow, green, blue) with sky blue as the base. Content is displayed on white cards with rounded corners and soft shadows. Text elements use theme-aware utilities with backdrop blur effects for enhanced readability over the rainbow background. The design is fully responsive, accessible (WCAG AA compliant) with support for both light and dark modes, high contrast, keyboard navigation, and utilizes Framer Motion for smooth spring animations.
