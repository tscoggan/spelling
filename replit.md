...
- **Dictionary Integration**: Uses a two-tier Free Dictionary lookup by default: primary v1 endpoint (`freedictionaryapi.com/api/v1`, the official documented API with broader word coverage) with automatic fallback to v2 (`api.dictionaryapi.dev/api/v2`) only when v1 has a transient error. Both are key-free. The two endpoints have different JSON shapes, so each has its own parser (`parseFreeDictionaryV1Response` for v1, `parseFreeDictionaryResponse` for v2). Merriam-Webster APIs (Learner's + Collegiate) are preserved as a config-switchable alternative; activate by setting `DICTIONARY_SOURCE = 'merriam-webster'` in `server/services/dictionaryConfig.ts`.
