...
### UI/UX Decisions
The application features a vibrant, kid-friendly interface with a colorful crayon-style title banner and playful school-themed background decorations (cartoon pencils, bookworms, books, and classroom objects). Large buttons, generous spacing, and a cheerful color palette (Primary: Purple, Secondary: Yellow, Accent: Green, Background: Light Blue-tinted White with decorative patterns, Foreground: Dark Blue) create an engaging learning environment. Simple kid-friendly words (cat, dog, apple, book, sun, house) display with colorful cartoon illustrations during gameplay. The design uses Nunito typography for readability and is responsive across mobile, tablet, and desktop devices. Smooth animations via Framer Motion enhance user experience, while accessibility remains a priority with high contrast, keyboard navigation, and screen reader support (WCAG AA compliant).
...
- **Cartoon Illustrations**: Simple kid-friendly words (cat, dog, apple, book, sun, house) display colorful cartoon illustrations during gameplay. Images appear between the instruction text and play button with spring animations. The word-to-image mapping system easily allows adding more illustrated words in the future.
