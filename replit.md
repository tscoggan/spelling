...
- API base URL is centralized in `client/src/lib/apiBase.ts`. On web it is empty (relative/same-origin). On native (Capacitor) it prefers the build-time `VITE_API_BASE_URL`, and falls back to the hardcoded production URL `https://spellingplayground.com` if that env var was omitted at build time — so the iPad/Android app still reaches the backend even when the Mac Mini build forgets to `export VITE_API_BASE_URL`. The native app uses the custom domain `spellingplayground.com`, which now has a valid Let's Encrypt TLS certificate. The certificate was originally missing because the required `replit-verify` TXT record was absent from DNS even though the Replit Domains panel showed "Verified"; it was fixed by adding the TXT record at the registrar and re-linking the domain in the Replit Domains panel to force a fresh certificate request. If the custom domain's certificate ever breaks again, the documented fallback is the Replit-managed domain `https://spell-champ-tgs4.replit.app`, which always has a valid certificate.
...
- **Object-storage images need `assetUrl()` on native.** Avatars and word-list illustrations are stored/served as RELATIVE paths `/objects/images/{hash}.{ext}` (via `GET /objects/:objectPath(*)`). `<img>`/`<audio>` tags never pass through the `native-fetch.ts` interceptor (it only patches `window.fetch`), so on native a relative `/objects/...` src resolves to `capacitor://localhost/objects/...` and 404s — broken avatars (header, leaderboard, user-groups, head-to-head, home) and broken word pictures (word-lists list + in-game illustration). The helper `assetUrl()` in `client/src/lib/apiBase.ts` prepends `API_BASE` to `/objects/` and `/api/` paths and is applied at every image render site; it is a no-op on web and passes absolute (Pixabay CDN) and `data:` URLs through unchanged.
- **Native internal navigation uses wouter, not hard reloads.** After account deletion the app navigates with wouter `setLocation("/auth")` instead of `window.location.href = "/auth"`; a hard reload to a relative route can fail to load from the Capacitor bundle (white screen) on native.
- **Native session cookies require CapacitorHttp.** `capacitor.config.ts` enables `CapacitorHttp` and `CapacitorCookies` plugins so all fetch/XHR go through the native HTTP stack with a native cookie jar. Authentication is cookie-based (`secure: true; sameSite: none` in production, with `trust proxy` set in `server/auth.ts`). Because the native WebView origin is `capacitor://localhost` while the API is `https://spellingplayground.com`, the session cookie is third-party and iOS WKWebView silently drops it — so without CapacitorHttp every authenticated request returns 401 (symptom seen: family signup returned 200 but the immediate `POST /api/family/send-email-verification` returned 401, so the 6-digit verification email never sent). Enabling CapacitorHttp stores Set-Cookie in the native jar and re-sends it, fixing all native auth. This is config-only (CapacitorHttp is built into `@capacitor/core` v8) but requires a Mac Mini rebuild (`npx cap sync ios` then rebuild in Xcode) to take effect on device.
