...
- **Three-Tier Account System**: 
  ...
  - **School**: Full-featured accounts with complete access to all features including social features, user groups, and head-to-head challenges. School account creation follows a 3-step wizard: (1) Admin account details + school name, (2) Simulated $0.99 adult verification payment, (3) Done. School admin (`role: "school_admin"`) manages teachers (`role: "teacher"`) and students (`role: "student"`) from the School Dashboard at `/school`. School account data is stored in `school_accounts` and `school_members` tables. Routes: `POST /api/school/signup`, `POST /api/school/payment/confirm`, `GET /api/school/account`, `POST /api/school/teachers`, `POST /api/school/students`, `DELETE /api/school/members/:memberId`.
