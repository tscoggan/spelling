...
- **Game Modes**:
    ...
    - **Crossword Puzzle**: Interactive, audio-only crossword puzzles with client-side grid generation, prioritizing word connectivity. Limited to 15 words for grid generation (mode-specific). Implements horizontal scrolling (`overflow-x-auto`) for grids that exceed screen width.
