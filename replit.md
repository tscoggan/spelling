...
- **Legal Compliance System**: Clickwrap acceptance for all three required school documents — School Terms of Service, Student Data Privacy Addendum, and COPPA Certification — on a dedicated legal step during signup. Stub placeholder pages exist at `/legal/school-tos`, `/legal/student-dpa`, and `/legal/privacy-policy` (full legal text to be provided). Acceptance is logged in the `agreement_acceptances` table with encrypted IP, user agent, text hash, and version for every agreement. Legal document links are permanently accessible from the school dashboard footer. `shared/agreements.ts` (server-only) stores versioned text and SHA256 hashes for: `coppa_certification` v1.1, `school_tos` v1.0, `student_dpa` v1.0.
