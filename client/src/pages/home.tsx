          ...
          <div className="flex-1 overflow-y-auto space-y-1 pr-2">
            {allLists.length === 0 ? (
              ...
              allLists.map((list) => (
                <div
                  key={list.id}
                  className="flex items-center gap-0.5 p-1 rounded-md border hover-elevate active-elevate-2 cursor-pointer"
                  onClick={() => startGameWithCustomList(list)}
                  data-testid={`card-word-list-${list.id}`}
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-0.5">
                      <div className="font-semibold text-sm leading-tight truncate" data-testid={`text-list-name-${list.id}`}>
                        {list.name}
                      </div>
                      {list.gradeLevel && (
                      ...
                      {list.isPublic ? (
                        <Globe className="w-3 h-3 text-blue-600 flex-shrink-0" data-testid={`icon-public-${list.id}`} />
                      ) : (
                        <Lock className="w-3 h-3 text-gray-400 flex-shrink-0" data-testid={`icon-private-${list.id}`} />
                      )}
                    </div>
